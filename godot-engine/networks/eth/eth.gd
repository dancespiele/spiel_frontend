...
func init_icon():
	var metamask = Button.new()
	metamask.icon = ResourceLoader.load("res://assets/metamask_icon.png")

	var metamask_initial_position := Vector2($WalletConnect.position.x + 180, $WalletConnect.position.y)

	metamask.set_position(metamask_initial_position)
	metamask.scale = Vector2(0.135, 0.135)

	metamask.connect('pressed', _on_wallet_connect_pressed)
	add_child(metamask)
...
func connect_handled():
	if provider and !$WalletConnect.visible:
...
func get_signature_callback(args):
	...
	var endpoint = "{endpoint}/login".format({"endpoint": backend_url})
...
func check_prizes(token: String):
	var endpoint = "{endpoint}/prize".format({"endpoint": backend_url});
